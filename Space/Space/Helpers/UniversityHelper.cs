...
namespace Space.Helpers
{
    public static class UniversityHelper
    {
        ...
        public static University GetRandomMonkey()
        {
            //var output = Newtonsoft.Json.JsonConvert.SerializeObject(Monkeys);
            return Universities[random.Next(0, Universities.Count)];
        }

        public static ObservableCollection<Grouping<string, University>> UniversitiesGrouped { get; set; }

        public static ObservableCollection<University> Universities { get; set; }

        static UniversityHelper()
        {
            random = new Random();
            ...
            var sorted = from university in Universities
                         orderby university.Name
                         group university by university.NameSort into universityGroup
                         select new Grouping<string, University>(universityGroup.Key, universityGroup);

            UniversitiesGrouped = new ObservableCollection<Grouping<string, University>>(sorted);
        }
    }
}
